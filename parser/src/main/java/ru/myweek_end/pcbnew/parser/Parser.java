...
public class Parser {
	...
	/**
	 * Разбор текста на объекты
	 * @since 0.0.1.2
	 * @param source Текст
	 * @return Объект Pcbnew
	 * @throws IOException При проблемах чтения
	 */
	public PcbnewObject parse (Reader source) throws IOException {
		Context context = new Context();
		context.setState(new State0());
		context.setCurrentObject(new PcbnewObject(null));
		BufferedReader reader = new BufferedReader(source);
		try {
			int value;
			while((value = reader.read()) != -1) {
				char character = (char)value;
				context = context.getState().step(character, context);
			}
			context = context.getState().step(' ', context);
		} finally {
			reader.close();
		}
